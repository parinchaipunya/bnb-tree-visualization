...
def generate_tree_layout(raw_nodes, edges):
    max_char_length = 0
    ...
    auto_spacing_x = max(2.5, max_char_length * 0.15) 
    auto_spacing_y = auto_spacing_x * 0.8 
    ...
    def assign_coords(node_id, depth=0):
        ...
        if not children[node_id]:
            positions[node_id] = (current_leaf_x, -depth * auto_spacing_y)
            current_leaf_x += auto_spacing_x
        else:
            child_xs = []
            ...
            positions[node_id] = (avg_x, -depth * auto_spacing_y)
...
def draw_bnb_incremental(all_nodes, all_edges, reveal_order, filename, title_prefix="B&B Step", savepng=True, showplt=True):
    PALETTE = {
        "root": "lightgray", "explored": "lightgray",
        "fractional": "lightblue", "incumbent": "lightgreen",
        "pruned": "salmon", "infeasible": "salmon"
    }
    ...
    data_width = 2 * max_offset
    num_levels = len(set([round(y, 3) for y in all_y]))
    
    inches_per_x_unit = 0.6
    inches_per_y_level = 1.5
    
    calculated_width = data_width * inches_per_x_unit
    calculated_height = num_levels * inches_per_y_level
    
    canvas_width = max(16.0, calculated_width)
    canvas_height = max(9.0, calculated_height)
    
    figsize = (canvas_width, canvas_height)
    
    x_padding = max_offset * 0.05 if max_offset > 0 else 0.5
    x_min = root_x - max_offset - x_padding
    x_max = root_x + max_offset + x_padding

    y_padding = (max(all_y) - min(all_y)) * 0.05 if max(all_y) != min(all_y) else 0.5
    ...
    for i, group in enumerate(reveal_order):
        ...
        for u, v, branch_txt in all_edges:
            if u in visible_nodes and v in visible_nodes:
                ...
                ax.text(mid_x, mid_y, branch_txt, fontsize=9, 
                        ha='center', va='center', 
        ...
        for nid in visible_nodes:
            ...
            color = "lightgray" if nid in currently_has_children else PALETTE.get(current_status.lower(), "white")
            
            ax.text(pos[0], pos[1], full_label, ha='center', va='center', fontsize=8,
        ...
        if savepng:
            plt.savefig(f"{filename}_{i+1}.png", bbox_inches='tight', dpi=150)
        if showplt:
            plt.show()
